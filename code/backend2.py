...
def upload_file():
    ...
    if f_name.startswith('Random Forest'):
        arith=0
    elif f_name.startswith('PCA'):
        arith=1
    elif f_name.startswith('SVM'):
        arith=2
    elif f_name.startswith('BPN'):
        arith=3
    elif f_name.startswith('BP'):
        arith=4
    elif f_name.startswith('RBF'):
        arith=8
    elif f_name.startswith('GMM'):
        arith=6
    elif f_name.startswith('LDA'):
        arith=7

    data = {"code": 500, "msg": "上传失败！"}
    try:
        file_buffer.save(f_name)
        data.update({"code": 200, "msg": "上传成功！"})
    except FileNotFoundError as e:
        print("error", e)
    return json.dumps({"tr":1,"recommend":arith})
   ...
   # print(arith)
    ...
    if judgestate==1:
        ...
        if arith=='Random Forest':
            ...
            pca = PCA(n_components=int(varNumber),svd_solver='randomized',whiten=True).fit(train_x)#??????????
            ...
            x_train_Standard = scaler.transform(train_x)
            clf.fit(x_train_Standard, train_y)
        elif arith=='RBF':
            clf = SVR(kernel='rbf')
            clf.fit(train_x,train_y)
        elif arith=='GMM':
            clf = GaussianMixture(n_components=int(varNumber))
            clf.fit(train_x,train_y)
        elif arith=='LDA':
            clf = LDA(n_components=int(varNumber))
            clf.fit(train_x, train_y)
        #elif arith=='CNN':
        ...
        if rownum<(len(test_x)):
            ...
            if arith=='Random Forest':
                ...
                scaler = StandardScaler()
                scaler.fit(sample)  # 训练标准化对象
                x_test_Standard = scaler.transform(sample)
                y_pre=clf.predict(x_test_Standard)
            elif arith == 'RBF':
                y_pre = int(clf.predict(sample))
            elif arith == 'GMM':
                y_pre = clf.predict(sample)
            elif arith == 'LDA':
                y_pre = clf.predict(sample)
        ...
        for i in range(1,int(varNumber)):
            ...
            stasize=len(te_input['train_x'])*int(varNumber)
            print(stasize)
            if stasize>=100000:
                return json.dumps({"recommend":0})
            elif stasize>=60000:
                return json.dumps({"recommend":random.randint(3,5)})
            elif stasize>=45000:
                return json.dumps({"recommend": 8})
            elif stasize>=35000:
                return json.dumps({"recommend": 6})
            elif stasize>=10000:
                return json.dumps({"recommend": 0})
            elif stasize>=8000:
                return json.dumps({"recommend": 7})
            elif stasize>=5000:
